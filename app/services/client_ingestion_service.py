...
def normalize_val(val: Any) -> Any:
    ...
    if isinstance(val, str):
        s_val = val.strip()
        # If it's a numeric string, normalize to float
        try:
...
class ClientIngestionService:
    ...
    async def process_records(
        ...
        # 1. Fetch existing clients from DB
        query = select(ClientMaster).where(
            ClientMaster.business_id == business_id,
            ClientMaster.is_deleted == False
        )
        db_result = await db.execute(query)
        existing_clients = db_result.scalars().all()

        # Map existing clients by composite key (client_id.strip(), category.strip())
        db_map: Dict[Tuple[str, str], ClientMaster] = {
            (c.client_id.strip(), c.category.strip()): c for c in existing_clients
        }
        ...
        for idx, row in enumerate(records):
            ...
            # Map derived revenue calculations
            if ("revenue" not in c_row or c_row["revenue"] is None or str(c_row["revenue"]).strip() == ""):
                ...
                if is_sub and c_val:
                    try:
                        c_row["revenue"] = round(float(c_val) / 12.0, 2)
                    except (ValueError, TypeError):
                        pass

            raw_client_id = c_row.get("client_id")
            raw_category = c_row.get("category")

            # Check missing required fields
            validation_result = ClientValidationService.validate_preview_row(c_row)
            missing_fields = validation_result.missing_required_fields

            if missing_fields:
                failed_count += 1
                results.append({
                    "client_id": str(raw_client_id) if raw_client_id else f"ROW_{idx+1}",
            ...
            try:
                ...
                revenue_val = float(c_row.get("revenue", 0))
            except (ValueError, TypeError):
                failed_count += 1
                results.append({
                    "client_id": client_id,
                    ...
                    "action": "INVALID",
                    "status": "failed",
                    "reason": "Invalid numeric value for contract_value or revenue"
                })
                continue
            ...
            rec_data = {
                "client_id": client_id,
                "client_name": str(c_row.get("client_name", "")).strip(),
                "category": category,
            ...
            if existing_db_obj:
                ...
                if not changed_fields:
                    ...
                    results.append({
                        ...
                        "action": "UPDATED",
                        "status": "updated",
                        "reason": "Existing client data changed",
                ...
                results.append({
                    ...
                    "action": "INSERTED",
                    "status": "inserted",
                    "reason": "New Client ID + Category combination"
                })
        ...
        try:
            ...
            await db.rollback()
            logger.error(f"Error committing client ingestion: {e}")
            raise e
